...
- **Dependency classification and predecessor condition:** <!-- Higher-risk: independent, or ordered with the checkpoint-defined predecessor condition and a link to its checkpoint satisfaction record. Ordinary: Not required. -->
...
- [ ] An authorized Higher-risk slice is classified as independent or ordered; an ordered slice has not begun before its recorded predecessor condition and pre-start checkpoint preservation check are satisfied.
- [ ] For Higher-risk work, the planning base, authority, owner, scope, old-path disposition, and slice still match the accepted checkpoint; otherwise the checkpoint was revised and readiness renewed before implementation continued.
