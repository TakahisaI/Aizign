...
The implemented contract for the committed-prefix JSONL store is store
...
The production runtime enforces that v2/profile contract. A complete v2 store
is fenced from a historical v1 binary by its `storeVersion: 2` commit marker.
Initialization interrupted before that marker is durable is not fenced,
remains unsupported, and is operator-discard-only. There is no silent v1
adoption, migration, dual reader, or repair path.
