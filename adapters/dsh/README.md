...
| **Inputs** | plugin config（binary、stateDir、timeoutMs、eventId + workflow / assignment / attempt / candidate digest + optional repair source）、agentのtool call `{ kind, findingCount?, artifactRef?, evidenceDigest?, shortErrorCode? }` |
...
- id: aizign-workflow-signal
  ...
  config:
    ...
    # repair assignmentだけ指定。agentへは公開されない
    # sourceEventId: evt-review-findings-01
```
...
最後の `tool/call`（このtool）と対になる `tool/result` を探して、`eventId` と `bindingDigest` を plugin config から再計算した値と照合します。binding digestにはattempt、candidate digest、optional repair sourceも含まれます。
