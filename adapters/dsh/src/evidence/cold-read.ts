...
export interface ColdReadOptions {
  ...
  /** Optional metadata-only timing sink. Sink failures are ignored. */
  readonly timingSink?: ColdReadTimingSink;
}

export const DEFAULT_MAX_EVENTS = 10_000;
export const DEFAULT_COLD_READ_TIMEOUT_MS = 10_000;

/** One harness-native cold-read observation, kept separate from journal authority. */
export interface ColdReadTimingMeasurement {
...
export async function readSignalEvidence(
  ...
  if (result === undefined)
    return finish({ kind: 'unknown', reason: 'no_result', callSeq: call.seq });
  ...
  if (
    ...
    return finish({ kind: 'unknown', reason: 'meta_mismatch', callSeq: call.seq });
  }
  return finish({
