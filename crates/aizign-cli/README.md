...
| | |
|---|---|
| **Responsibility** | 引数、stdin / stdout、system clock、state directoryの選択、`JsonlJournal` と `aizign-engine` の結線、処理時間のbound、stderrへの構造化log |
...
`hello` responseの `journalSchemaVersion` は `aizign-store-jsonl` の定数から、`package.version` はこのcrateのversionから取ります。
