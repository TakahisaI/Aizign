//! The `aizign` binary: the composition root and the only place in the
//! workspace that touches arguments, stdin/stdout, the system clock, and the
//! state directory.
//!
//! ```text
//! aizign hello                    # handshake response, no state needed
//! aizign handle --state <dir>     # one request frame on stdin -> one response frame on stdout
//! ```
//!
...
const USAGE: &str = "\
usage:
  aizign hello                    print the hello response (protocol version, capabilities)
  aizign handle --state <dir>     read one request frame from stdin, write one response frame to stdout
  aizign --version
";
...
fn main() -> ExitCode {
    ...
    let code = match args.first().map(String::as_str) {
        ...
        Some("--version" | "-V") => {
            println!("aizign {}", env!("CARGO_PKG_VERSION"));
            exit::OK
        }
        Some("--help" | "-h" | "help") => {
            print!("{USAGE}");
