...
- 初期実装はCIとopen-flag ABIを検証した `x86_64-unknown-linux-gnu` 専用。x86_64 GNU/Linux CIでfile / directory barrier、atomic replace、lock、owner-only mode、artifact type / linkを検証する。別architecture / libcのLinux、macOS、BSD、Windowsを含む未検証targetは `Unavailable` でfail closedし、CLIもstore capabilityをadvertiseしない
