...
- 初期実装は `target_os = "linux"` 専用。Linux CIでfile / directory barrier、atomic replace、lock、owner-only mode、artifact type / linkを検証する。macOS・BSD・Windowsを含む未検証platformは `Unavailable` でfail closedし、CLIもstore capabilityをadvertiseしない
- lock / journal / commit / temporary commitはLinuxの`O_NOFOLLOW`で開き、open前後のdevice / inode、regular file、link count 1、state directoryと同じowner、owner-only modeを検査する。temporary commitは検査済みのstale regular fileだけを除去し、`create_new`で作る
