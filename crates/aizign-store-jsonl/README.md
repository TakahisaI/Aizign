...
| **Hard invariants** | attempt / candidate digestをmetadata-only recordへ残す（5）、本文・credential・harness IDを書かない（10）、write開始後のbarrier / publish失敗は `OutcomeUnknown` で再送しない（3）、readerはwrite / sync / initialize / repair / tail promotionを一切しない（9）、published prefix以外をacceptedの根拠にしない、既存recordを書き換えない |
...
- 初期実装はdurable directory barrierとowner-only modeを検証できるUnix専用。未検証platformは `Unavailable` でfail closedする。LinuxをCIで検証する
