...
Application engine around `aizu-core`: use cases, and the ports the shell must supply.
...
| **Non-responsibility** | 判断（`aizu-core`）、wire format（`aizu-protocol`）、I/Oの実装（store、cli）、harness固有型 |
...
| **Allowed dependencies** | `aizu-core`。dev: `aizu-testkit` |
| **Test command** | `cargo test -p aizu-engine` |
