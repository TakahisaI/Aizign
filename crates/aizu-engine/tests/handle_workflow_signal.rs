...
use aizu_core::workflow::{ApplyError, Command, WorkflowError, WorkflowEvent};
use aizu_engine::{ClockError, HandleError, JournalError, SignalOutcome, handle_workflow_signal};
use aizu_testkit::{FixedClock, MemoryJournal, signals};
...
fn inconsistent_journals_are_corrupt_not_reinterpreted() {
    ...
    aizu_engine::Journal::append(&mut journal, &event, signals::at(0)).unwrap();
    aizu_engine::Journal::append(&mut journal, &event, signals::at(1)).unwrap();
