...
/// member name at any object depth. Other syntax errors remain the typed
/// decoder's responsibility.
pub(crate) fn has_duplicate_members(frame: &[u8]) -> bool {
...
mod tests {
    ...
    fn finds_decoded_names_at_every_depth() {
        assert!(has_duplicate_members(br#"{"a":1,"a":2}"#));
        assert!(has_duplicate_members(br#"{"a":1,"\u0061":2}"#));
        assert!(has_duplicate_members(br#"[{"a":1,"a":2}]"#));
        assert!(has_duplicate_members(br#"{"outer":{"a":1,"a":2}}"#));
        assert!(!has_duplicate_members(br#"{"a":1,"b":{"a":2}}"#));
    }
}
