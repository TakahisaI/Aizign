...
4. Return the decoded entries without changing file contents, length, metadata documents, mtime, or durability state. OS-managed access time (`atime`) is filesystem-policy-dependent and is not part of this contract.

A valid published snapshot is required before reconciliation can return `absent`. Only a valid zero-entry commit point establishes an authoritative empty snapshot; a valid non-empty snapshot also returns `absent` when the queried event ID is not present. A submission failure after durable initialization but before the first append can therefore reconcile as `absent`, while a failure before initialization remains `unknown`. A complete record left after `write_all` succeeded and `sync_data` failed is beyond the published boundary and remains `unknown`, even after restart.
...
Directory durability is part of the store contract, not an optional optimization. The initial implementation supports only targets on which it implements and tests equivalent file synchronization, atomic replacement, parent-directory synchronization, state-directory synchronization, and the architecture-specific open-flag ABI. On any other target it fails closed with `JOURNAL_UNAVAILABLE`; it must not silently downgrade the durability guarantee. The normative barrier sequence above is implemented and advertised initially only for `x86_64-unknown-linux-gnu`, with that target covered in CI. Other Linux ABIs, architectures, or libc environments and non-Linux builds omit the submit and reconciliation capabilities and reject direct requests with `CAPABILITY_UNSUPPORTED`. The x32 ABI is intentionally unsupported: CI only cross-compiles it as a negative boundary proving that it cannot be mistaken for the supported 64-bit target. This decision adds no x32 durability implementation, runtime contract tests, release artifacts, or support claim. Additional target support requires a separate decision and target-specific contract tests before it is claimed.
...
- Binary downgrade against the same state directory is unsupported and is not technically fenced. An old binary can ignore the commit document and append beyond its boundary; operators must not open a new-layout state directory with such a binary. Returning to the new binary detects the resulting extra tail as `JOURNAL_OUTCOME_UNKNOWN`, but that detection does not make the downgrade itself fail closed.
...
- An old binary can mutate a new-layout state directory because it does not know the commit document. Same-state binary downgrade is operationally prohibited but not technically prevented.
