...
| Oversized request or response | Detected and fail closed | Production encoders, CLI, and clients | Refuse outbound frames or reject inbound frame bodies before a stronger classification; validate but do not retain permitted whitespace after LF | Encoder bounds, CLI framing, exact-max padded-frame coverage, and core-client conformance tests | Timeout bounds caller wait, not all remote work |
