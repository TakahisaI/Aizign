...
| DSH logical submit is repeated after lost acknowledgement or restart | Not guaranteed | Future ADR-0029 DSH lifecycle owner | Accepted target: durably publish the exact attempt before spawn, recover `in_flight` as reconciliation-required, and keep submit gated until a terminal lifecycle result | Lifecycle-v1 exact 134-case inventory; executable evidence is required in S2 | Current DSH runtime has no durable lifecycle fence; multi-process sharing and operator discard/reset remain unsupported |
