...
|---|---|---|---|---|---|
| Malformed UTF-8, BOM, duplicate member, invalid Unicode, non-canonical number, unknown field or kind | Runtime enforced | Rust and TypeScript protocol codecs and process clients | Keep process stdout as bytes through frame extraction and fatal decode; reject before domain handling or a stronger client classification | Shared protocol/schema fixtures and raw invalid-UTF-8 core-client fault scenarios | A valid opaque field can still carry semantically inappropriate content |
| Oversized or noncanonical request/response process stream | Detected and fail closed | Production encoders, CLI, and clients | Refuse outbound frames; require one bounded body + LF + immediate close; reject CRLF and every post-LF byte before a stronger classification | Encoder bounds, shared process-profile cases, exact-bound and post-LF-negative CLI/core-client tests | Timeout bounds caller wait, not all remote work |
...
| Partial write, barrier failure, PREPARED generation, corrupt state, or unpublished tail | Detected and fail closed | JSONL store | Store v2 orders durable PREPARED, journal barrier, durable commit namespace, then CLEAN; uncertain append images remain `OutcomeUnknown`; readers use only a fully revalidated CLEAN prefix | Exact 38-case production executor plus runtime mutation/barrier evidence | A same-user attacker can forge all mutually consistent files; real SIGKILL/power-loss evidence remains Issue #82 |
...
| DSH logical submit is repeated after lost acknowledgement or restart | Not guaranteed | Future ADR-0029 DSH lifecycle owner | Accepted target: durably publish the exact attempt before spawn, recover `in_flight` as reconciliation-required, and keep submit gated until a terminal lifecycle result | Lifecycle-v1 exact 112-case inventory; executable evidence is required in S2 | Current DSH runtime has no durable lifecycle fence; multi-process sharing and operator discard/reset remain unsupported |
| Lifecycle root is replaced, aliased, mounted on an unsupported profile, or shared by another process | Not guaranteed | Future DSH lifecycle qualifier and trusted operator | Accepted target: separate mount/descriptor qualification, root marker binding, process-local owner lease, fail-closed artifact validation | Profile, initialization, lease, restart, mutation, and non-disclosure lifecycle cases in S2 | Same-user namespace honesty, backing persistence, root retention, and single-process operation remain trusted assumptions |
...
| Old v1 binary opens complete v2 state | Detected and fail closed | Store-version probe plus operator procedure | The retained commit path carries `storeVersion: 2`, which v1 rejects; pre-marker partial initialization is discarded by the operator | Compatibility/store specification and implemented downgrade tests | Pre-marker partial initialization is not technically fenced |
| Unsupported storage profile receives a weaker durability contract | Runtime enforced | Store profile gate and CLI capability handling | Require exact `linux-x86_64-gnu-ext4-local-v1`; path failure is `JOURNAL_UNAVAILABLE` and never known | Store-v2 profile corpus, exact-profile runtime evidence, and x32 negative boundary | Backing persistence and honest barrier behavior remain operator-trusted |
...
- Production implements store v2 and enforces the exact qualified profile.
  x32 remains only a compile-time negative boundary.
...
- ADR-0029's DSH lifecycle profile is an accepted future target, not current
  v0.1 support. It is limited to the exact Linux x86_64 glibc/ext4-local
  profile in `spec/dsh/lifecycle/v1/`; other platforms and multi-process root
  sharing are unsupported, and S1 creates no runtime enforcement.
