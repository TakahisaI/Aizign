...
| Store path or operating-system detail in a Protocol error reaches the model-facing DSH tool error | Runtime enforced | DSH submit outcome mapping | Preserve the stable code but replace rejected/unknown diagnostic text with a fixed safe message | DSH tool mapping tests use synthetic private-path detail and assert it is absent | Direct trusted `CoreClient` consumers still receive the operational Protocol message and must apply their own presentation policy |
