...
| | |
|---|---|
| **Responsibility** | fake core process（Protocol v1、JSON state、fault injection）、`runCoreClientConformance`、`ReferenceOneShotClient`（参照実装）、`assertMetadataOnly`、`samplePayload` |
...
├── conformance.ts         runCoreClientConformance(factory)、samplePayload、assertMetadataOnly、FORBIDDEN_KEYS
