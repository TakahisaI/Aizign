...
| `signal` | object | 受理されたstructured signal（closed。attempt、candidate digest、external evidence digest、repair causationをdurableに含む。protocol v1の `signal` と同じ形だが別schemaとして所有） |
...
- `signal` は `attemptId` とtyped `candidateDigest`を必須にする。`artifactRef` / `evidenceDigest`は一体で、repairの`sourceEventId`も保存する
- `signal` の条件規則（kindとroleの対応、`findingCount` / artifact pair / `sourceEventId` / `shortErrorCode` の必須・禁止）は [`record.schema.json`](schemas/record.schema.json) がprotocol v1のrequest schemaと同じ形で持つ
...
- replay時に同じrevision / artifact referenceが異なるdigestへbindされている、またはrepair sourceが未受理・別workflow・findings以外・消費済みなら `JOURNAL_CORRUPT`
